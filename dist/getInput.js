import { a as sendTimestamp } from "./assets/messages-Dil7ynbK.js";
function getCurrentTime() {
  ...
  return hours > 0 ? `${hours}:${minutes}:${padSeconds}` : `${minutes}:${padSeconds}`;
}
function getCleanURL() {
  const url = new URL(window.location.href);
  const videoId = url.searchParams.get("v");
  if (videoId.length <= 0) return void 0;
...
function injectHTML() {
  ...
  let youtimediv = document.querySelector("#youtime");
  if (!youtimediv) {
...
function getInput() {
  ...
  const textarea = document.querySelector("#youtime-input");
  const saveBtn = document.querySelector("#youtime-save-btn");
  if (saveBtn) {
